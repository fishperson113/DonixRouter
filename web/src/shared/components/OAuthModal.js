...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  ...
  const [importingSessionBlob, setImportingSessionBlob] = useState(false);
  ...
  const handleSessionBlobImport = async () => {
    const raw = sessionBlob.trim();
    if (!raw) {
      setError("Please paste a session JSON blob");
      return;
    }
    let parsed;
    try {
      ...
      setError("Invalid JSON. Make sure you copied the full session blob.");
      return;
    }
    if (!parsed?.accessToken) {
      setError('JSON is missing the "accessToken" field.');
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} title={`Connect ${providerInfo.name}`} onClose={handleClose} size={provider === "codex" ? "full" : "lg"}>
      <div className="flex flex-col gap-4">
        {/* Waiting + Manual Input combined (non-device-code) */}
        {(step === "waiting" || step === "input") && !isDeviceCode && (
          provider === "codex" ? (
            <>
              <div className="rounded-2xl border border-border bg-surface-2 dark:border-white/10 dark:bg-[radial-gradient(circle_at_top_left,rgba(16,163,127,0.12),transparent_55%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0))] p-4">
              ...
              <div className="surface-panel-soft flex items-center gap-2 rounded-xl border border-border dark:border-white/10 px-3 py-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="surface-panel rounded-2xl border border-border dark:border-white/10 p-4">
                  <div className="mb-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        rows={5}
                        className="w-full resize-none rounded-[14px] border border-border bg-surface-2 px-3 py-2 text-sm font-mono text-text-main transition-all duration-150 ease-out placeholder-text-muted/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-brand-500/40 focus:outline-none focus:ring-2 focus:ring-brand-500/30 dark:border-white/10 dark:bg-white/[0.04]"
                      />
                    </div>
              ...
              <div className="surface-panel rounded-2xl border border-white/10 p-4">
                <div className="mb-4 flex items-start gap-3">
                  <div className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 text-amber-400">
                  ...
                  <div className="min-w-0">
                    ...
                    <h4 className="mt-2 text-sm font-semibold">Import ChatGPT session JSON</h4>
                    <p className="mt-1 text-sm text-text-muted">
                      Paste the full JSON returned by ChatGPT (contains <span className="font-mono">accessToken</span>, <span className="font-mono">account</span>, <span className="font-mono">expires</span>). No refresh token, so the connection stops working when the access token expires.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <textarea
                    ...
                    placeholder='{"accessToken":"eyJ...","expires":"2026-08-18T...","account":{...},"user":{...}}'
                    rows={6}
                  ...
                  />
                  <Button
                    onClick={handleSessionBlobImport}
                    fullWidth
                    loading={importingSessionBlob}
                    disabled={!sessionBlob.trim()}
                  >
                    Add with Session JSON
                  </Button>
                </div>
              </div>
            ...
            <>
              ...
              <div className="surface-panel-soft flex items-center gap-2 rounded-lg border border-border dark:border-white/10 px-3 py-2">
                <span className="material-symbols-outlined text-base text-primary animate-spin">
                  progress_activity
                </span>
        ...
        {step === "waiting" && isDeviceCode && deviceData && (
          <>
            <div className="text-center py-4">
              ...
              <div className="surface-panel-soft mb-4 rounded-lg border border-border dark:border-white/10 p-4">
        ...
        {step === "success" && (
          <div className="text-center py-6">
            ...
            {sessionWarning && (
              <div className="mx-auto mb-4 max-w-md rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-left text-sm text-amber-200">

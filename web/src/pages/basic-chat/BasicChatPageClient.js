...
export default function BasicChatPageClient() {
  ...
  useEffect(() => {
    ...
    const applyInitialSelection = (nextState) => {
      queueMicrotask(() => {
        setActiveSessionId(nextState.sessionId);
        setActiveProviderId(nextState.providerId);
        setActiveModelId(nextState.modelId);
      });
    };

    const createInitialSession = (sessionValue, nextState) => {
      queueMicrotask(() => {
    ...
    if (sessions.length > 0) {
      ...
      applyInitialSelection({
        ...
        providerId: sessionModel?.providerId || savedProvider.providerId,
        modelId: sessionModel?.id || savedModel.id,
      });
      return;
    }
  ...
  return (
    <div className="relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-transparent text-white">
      <div className="relative mx-auto flex flex-1 h-full min-h-0 w-full max-w-4xl flex-col">
        <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3 lg:px-6">
          <div ref={modelMenuRef} className="relative">
            <button
              ...
              className="surface-panel-soft flex items-center gap-3 rounded-2xl px-4 py-3 text-left transition hover:bg-white/[0.05]"
            >
            ...
            {modelMenuOpen ? (
              <div className="surface-panel absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-[20px] shadow-2xl shadow-black/50">
                ...
                <div className="max-h-[60vh] overflow-y-auto p-2 custom-scrollbar">
                  {providerGroups.map((group) => (
                    <div key={group.providerId} className="surface-panel-soft mb-2 rounded-[16px] p-2">
                        ...
                        {group.models.map((model) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-[14px] border px-3 py-3 text-left transition ${isActive ? "border-brand-500/35 bg-brand-500/12 shadow-[0_10px_22px_rgba(229,106,74,0.12)]" : "border-white/8 bg-white/[0.03] hover:bg-white/[0.05]"}`}
                            >
                              <div className="flex items-start justify-between gap-3">
                                ...
                                {isActive ? <span className="material-symbols-outlined text-[18px] text-brand-300">check_circle</span> : null}
                              </div>
                            </button>
                          );
                        })}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="surface-panel-soft rounded-2xl px-4 py-3 text-sm text-white/80 transition hover:bg-white/[0.05]"
            >
              History
            </button>
        ...
        {historyOpen ? (
          <div ref={historyMenuRef} className="surface-panel absolute right-4 top-[72px] z-20 w-[min(360px,calc(100vw-2rem))] rounded-[20px] p-2 shadow-2xl shadow-black/50 lg:right-6">
            ...
            <div className="max-h-[48vh] space-y-2 overflow-y-auto p-1 custom-scrollbar">
              {sessionItems.length === 0 ? (
                <div className="surface-empty rounded-[16px] p-4 text-sm">
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-[16px] border px-3 py-3 text-left transition ${isActive ? "border-brand-500/35 bg-brand-500/12 shadow-[0_10px_22px_rgba(229,106,74,0.12)]" : "border-white/8 bg-white/[0.03] hover:bg-white/[0.05]"}`}
                  >
                    <div className="flex items-start justify-between gap-3">
        ...
        {loadError ? (
          <div className="surface-alert mt-4 rounded-[18px] px-4 py-3">
        ...
        <div className="flex flex-1 flex-col min-h-0">
            ...
            {currentMessages.length === 0 ? (
              <div className="flex min-h-[50vh] items-center justify-center px-4 text-center">
                <div className="max-w-xl space-y-4">
                  <div className="surface-panel-soft mx-auto flex size-16 items-center justify-center rounded-[20px] text-white/80">
              ...
              {currentMessages.map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-6`}>
                    <div className={`max-w-[min(88%,42rem)] ${isUser ? "surface-panel rounded-3xl px-5 py-3.5 text-white" : "text-white/90"}`}>
                      ...
                      {message.attachments?.length ? (
                        <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 mt-2">
                          {message.attachments.map((attachment) => (
                            <a key={attachment.id} href={attachment.dataUrl} target="_blank" rel="noreferrer" className="surface-panel-soft overflow-hidden rounded-[18px]">
          ...
          <div className="shrink-0 pt-2">
            {attachments.length > 0 ? (
              <div className="mx-auto mb-3 flex w-full max-w-3xl flex-wrap gap-2 px-4">
                {attachments.map((attachment) => (
                  <div key={attachment.id} className="surface-panel-soft flex items-center gap-2 rounded-full px-3 py-2">
            ...
            <div className="mx-auto w-full max-w-3xl px-4 pb-2">
              <div className="surface-panel rounded-[26px] px-3 pb-2 pt-3">
                <textarea
                  value={draft}
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={!activeModel || loadingData} className="rounded-full p-2 text-white/50 transition hover:bg-white/[0.05] hover:text-white">
                  ...
                  <div className="flex items-center gap-2">
                    {isSending ? (
                      <button type="button" onClick={handleStop} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/[0.08] p-2 text-white transition hover:bg-white/[0.14]">
                        <span className="material-symbols-outlined text-[16px]">stop</span>
                      </button>
                    ) : null}
                    <button onClick={sendMessage} disabled={!canSend} className={`flex h-8 w-8 items-center justify-center rounded-full transition ${canSend ? 'bg-[linear-gradient(180deg,#fb7f55_0%,#e56a4a_100%)] text-white shadow-[0_10px_22px_rgba(229,106,74,0.18)] hover:opacity-95' : 'bg-white/[0.08] text-white/30 cursor-not-allowed'}`}>

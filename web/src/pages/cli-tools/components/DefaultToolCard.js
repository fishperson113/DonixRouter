...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = [], cloudEnabled = false, tunnelEnabled = false }) {
  ...
  const replaceVars = (text) => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim()) 
      ? selectedApiKey 
      : (!cloudEnabled ? "sk_donixrouter" : "your-api-key");
    ...
    return (
      <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
        <input
          ...
          className="w-full flex-1 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40 sm:w-auto"
        />
        <button
          ...
          className={`shrink-0 px-3 py-2 rounded-lg border text-sm transition-colors ${
            hasActiveProviders
              ? "border-white/10 bg-white/[0.04] text-text-main hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer"
        ...
        {modelValue && (
          <>
            <button
              ...
              className="shrink-0 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 transition-colors hover:border-primary/35 hover:bg-white/[0.06]"
            >
        ...
        {canShowGuide() && tool.guideSteps.map((item) => (
          <div key={item.step} className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.value && (
                <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
                  <code className="w-full flex-1 truncate rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-mono shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:w-auto">
          ...
          <div className="mt-2">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.04] px-2 py-1 text-xs transition-colors hover:border-primary/35 hover:bg-white/[0.06]"
              >
            ...
            <pre className="surface-code overflow-x-auto rounded-lg p-4">

    ...
    <>
      <Card padding="xs" className="overflow-hidden">
        ...
        <div className="mt-4 flex flex-col gap-4 border-t border-white/8 pt-4">
            {/* Hosts */}
            {mitmHosts.length > 0 && (
          <div className="surface-panel-soft mt-2 rounded-md px-2 py-1.5">
            ...
            <div className="flex flex-col gap-0.5 text-[11px] text-text-muted px-1">
              <p>Toggle DNS to redirect {tool.name} traffic through DonixRouter via MITM.</p>
              {!dnsActive && (
            ...
            {tool.defaultModels?.length > 0 && (
              <div className="flex flex-col gap-2">
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      <input
                        ...
                        disabled={!dnsActive}
                    className={`w-full min-w-0 rounded-lg border border-white/10 bg-white/[0.04] py-2 pl-2 pr-7 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40 sm:py-1.5 ${!dnsActive ? "opacity-50 cursor-not-allowed" : ""}`}
                      />
                  ...
                  className={`rounded-lg border px-2 py-2 text-xs transition-colors sm:py-1.5 ${hasActiveProviders && dnsActive ? "border-white/10 bg-white/[0.04] hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer" : "cursor-not-allowed border-white/10 opacity-50"}`}
                    >
                      Select
                    </button>
                  </div>

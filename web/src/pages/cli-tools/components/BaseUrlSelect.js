...
const buildOptions = ({ requiresExternalUrl, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl, cloudEnabled, cloudUrl, savedPresets, withV1 }) => {
  ...
  if (!requiresExternalUrl) {
    const port = typeof window !== "undefined" ? window.location.port : UPDATER_CONFIG.appPort;
    const localUrl = wrap(`http://127.0.0.1:${port}`);
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        <select
          ...
          className="flex-1 min-w-0 rounded-lg border border-white/10 bg-white/[0.04] px-2 py-2 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40 sm:py-1.5"
        >
          {options.map((o) => (

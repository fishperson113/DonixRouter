...
export default function CopilotToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    if (status?.config && Array.isArray(status.config) && selectedModels.length === 0) {
      const entry = status.config.find((e) => e.name === "DonixRouter");
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="surface-panel-soft flex min-h-[28px] flex-wrap gap-1.5 rounded-lg px-2 py-1.5">
                    ...
                    <span key={model} className="inline-flex items-center gap-1 rounded-full border border-white/8 bg-white/[0.04] px-2 py-0.5 text-xs text-text-muted transition-colors hover:border-primary/30 hover:bg-white/[0.06]">
                            {model}

...
export default function ClaudeToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_donixrouter for localhost
                ...
                <div className="surface-panel-soft rounded-xl p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="surface-code block rounded-lg px-3 py-2 font-mono text-xs">npm install -g @anthropic-ai/claude-code</code>
                    </div>
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
                ...
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      <input type="text" value={modelMappings[model.alias] || ""} onChange={(e) => onModelMappingChange(model.alias, e.target.value)} placeholder="provider/model-id" className="w-full min-w-0 rounded-lg border border-white/10 bg-white/[0.04] py-2 pl-2 pr-7 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40 sm:py-1.5" />
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasDonixRouter} loading={restoring}>

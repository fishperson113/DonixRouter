...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any DonixRouter model entry (support multi-model: custom:DonixRouter-0, custom:DonixRouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:DonixRouter"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:DonixRouter"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:DonixRouter-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:DonixRouter-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:DonixRouter"))?.baseUrl && (
                  ...
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    {modelList.length > 0 && (
                      <div className="flex flex-col gap-0.5 mb-1">
                        {modelList.map((id) => (
                      <div key={id} className="surface-panel-soft flex items-center gap-1.5 rounded-lg px-2 py-1">
                    ...
                    className={`shrink-0 rounded-lg border px-2 py-1.5 text-xs transition-colors ${hasActiveProviders ? "border-white/10 bg-white/[0.04] hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer" : "cursor-not-allowed border-white/10 opacity-50"}`}
                      >
                        Select
                      </button>
                  <button onClick={addModel} disabled={!modelInput.trim()} className="shrink-0 rounded-lg border border-white/10 bg-white/[0.04] px-2 py-1.5 text-xs transition-colors hover:border-primary/40 hover:bg-white/[0.06] disabled:opacity-50" title="Add model">
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasDonixRouter} loading={restoring}>

        ...
        <div className="mt-4 flex flex-col gap-4 border-t border-white/8 pt-4">
              ...
              {tool.defaultModels.map((model) => (
                <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <button
                    ...
                    className={`w-full whitespace-nowrap rounded-lg border px-2 py-2 text-xs transition-colors sm:w-auto sm:shrink-0 sm:py-1.5 ${hasActiveProviders ? "border-white/10 bg-white/[0.04] text-text-main hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer" : "cursor-not-allowed border-white/10 opacity-50"}`}
                  >
                    Select
                  </button>
                </div>
          ...
          {!isRunning && serverIsWindows && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-yellow-500/10 text-yellow-600 border border-yellow-500/20">
              ...
              <span>Windows: Run terminal (DonixRouter) as Administrator to enable MITM</span>
            </div>
          )}
          ...
          {!isRunning && (
            <div className="flex flex-col gap-1.5 px-1">
              <p className="text-xs text-text-muted">
                <span className="font-medium text-text-main">How it works:</span> Intercepts Antigravity traffic via DNS redirect, letting you reroute models through DonixRouter.
              </p>
              <div className="flex flex-col gap-0.5 text-[11px] text-text-muted">
                <span>1. Generates SSL cert & adds to system keychain</span>
                <span>2. Redirects <code className="rounded bg-white/[0.05] px-1 text-[10px]">daily-cloudcode-pa.googleapis.com</code> → localhost</span>
                <span>3. Maps Antigravity models to any provider via DonixRouter</span>
              </div>
            </div>
          )}
        </div>

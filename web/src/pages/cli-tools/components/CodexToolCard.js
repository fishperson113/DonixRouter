...
const parseSubagentModelFromConfig = (config) => {
  const subagentSectionMatch = config.match(/\[agents\.subagent\]([\s\S]*?)(?:\n\[|\s*$)/);
  if (!subagentSectionMatch) return "";

  const modelMatch = subagentSectionMatch[1].match(/^\s*model\s*=\s*"([^"]+)"/m);
...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    if (codexStatus?.config) {
      ...
      setSubagentModel(parseSubagentModelFromConfig(codexStatus.config));
    }
  }, [codexStatus]);
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_donixrouter for localhost if no key, otherwise use selected key
...
const configContent = `# DonixRouter Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "donixrouter"

[model_providers.donixrouter]
name = "DonixRouter"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[agents.subagent]
description = "Default subagent role for delegated tasks."
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if DonixRouter is deployed on a remote server.</p>
                  </div>
                </div>
                ...
                <div className="surface-panel-soft rounded-xl p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="surface-code block rounded-lg px-3 py-2 font-mono text-xs">npm install -g @openai/codex</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="rounded bg-white/[0.05] px-1">codex</code> to verify.</p>
                    <div className="border-t border-white/8 pt-2">
                      <p className="text-text-muted text-xs">
                        Codex uses <code className="rounded bg-white/[0.05] px-1">~/.codex/auth.json</code> with <code className="rounded bg-white/[0.05] px-1">OPENAI_API_KEY</code>.
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <input type="text" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="provider/model-id" className="w-full min-w-0 rounded-lg border border-white/10 bg-white/[0.04] py-2 pl-2 pr-7 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40 sm:py-1.5" />
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <button
                    ...
                    className={`w-full whitespace-nowrap rounded-lg border px-2 py-2 text-xs transition-colors sm:w-auto sm:shrink-0 sm:py-1.5 ${activeProviders?.length ? "border-white/10 bg-white/[0.04] text-text-main hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer" : "cursor-not-allowed border-white/10 opacity-50"}`}
                  >
                    Select Model
                  </button>
                </div>

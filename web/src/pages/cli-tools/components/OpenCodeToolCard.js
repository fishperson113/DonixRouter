...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("donixrouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("donixrouter/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasDonixRouter) return "not_configured";
    const url = status.config?.provider?.["donixrouter"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `donixrouter/${activeModelToShow}`,
        agent: {
          explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["donixrouter"]?.options?.baseURL && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="surface-panel-soft flex min-h-[28px] flex-wrap gap-1.5 rounded-lg px-2 py-1.5">
                          ...
                          <span
                            ...
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs cursor-pointer transition-colors ${
                              model === activeModel
                                ...
                                : "border border-white/8 bg-white/[0.04] text-text-muted hover:border-primary/30 hover:bg-white/[0.06]"
                            }`}
                    ...
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                      <button onClick={() => setModalOpen(true)} disabled={!activeProviders?.length} className={`rounded-lg border px-2 py-1 text-xs transition-colors ${activeProviders?.length ? "border-white/10 bg-white/[0.04] text-text-main hover:border-primary/40 hover:bg-white/[0.06] cursor-pointer" : "cursor-not-allowed border-white/10 opacity-50"}`}>Add Model</button>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasDonixRouter} loading={restoring}>

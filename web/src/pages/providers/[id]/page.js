...
import { ensureModelsLoaded } from "open-sse/config/providerModels.js";
...
import DeepSeekLoginModal from "../components/DeepSeekLoginModal";
...
export default function ProviderDetailPage() {
  ...
  const [syncModelsError, setSyncModelsError] = useState("");
  ...
  const handleSyncKiroModels = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        alert(`Successfully synced ${data.modelsCount} models from Kiro`);
        // Reload page to refresh models
        window.location.reload();
      } else {
        setSyncModelsError(data.error || "Failed to sync models");
      }
    } catch (error) {
      console.log("Error syncing Kiro models:", error);
      setSyncModelsError("Network error");
    } finally {
  ...
  const handleUpdateKiroLimit = async (id, limitEnabled) => {
    try {
      const res = await fetch(`/api/providers/${id}/kiro-billing`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.log("Error updating Kiro billing limit:", error);
          ...
          <div key={conn.id} className="flex min-w-0 items-stretch">
            <div className="flex-1 min-w-0">
              <ConnectionRow
                ...
                onToggleKiroLimit={(limitEnabled) => handleUpdateKiroLimit(conn.id, limitEnabled)}
    ...
    return (
      <div className="flex flex-wrap gap-3">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-primary/40 px-3 py-2 text-xs text-primary transition-colors hover:border-primary hover:bg-primary/10 sm:w-auto"
        >
              ...
              <div className="flex flex-wrap gap-2">
                {notAdded.map((m) => (
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-white/8 bg-white/[0.03] text-xs text-text-muted hover:text-primary hover:border-primary/40 hover:bg-primary/10 transition-colors"
            ...
            <div className="flex flex-wrap gap-2">
              {disabledDisplayModels.map((m) => (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-dashed border-white/10 bg-white/[0.03] text-xs text-text-muted hover:text-primary hover:border-primary/40 hover:bg-primary/10 transition-colors"
                  title="Restore model"
                >
                  <span className="material-symbols-outlined text-[13px]">add</span>
                  {m.id}
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
        ...
        <Card>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              ...
              {/* {thinkingConfig && (
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="rounded-md border border-white/10 bg-white/[0.04] px-2 py-1 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40"
                  >
              ...
              {/* Round Robin toggle — sticky input always in DOM, visibility-toggled to prevent CLS */}
              <div className="relative flex items-center gap-2">
                <span className="text-xs text-text-muted font-medium shrink-0">Round Robin</span>
                <Toggle
                ...
                <div
                  ...
                  style={{
                    opacity: providerStrategy === "round-robin" ? 1 : 0,
                    pointerEvents: providerStrategy === "round-robin" ? "auto" : "none",
                    visibility: providerStrategy === "round-robin" ? "visible" : "hidden",
                  }}
                >
                  <span className="text-xs text-text-muted whitespace-nowrap">Sticky:</span>
                  <input
                    type="number"
                    min={1}
                    tabIndex={providerStrategy === "round-robin" ? 0 : -1}
                    value={providerStickyLimit}
                    onChange={(e) => handleStickyLimitChange(e.target.value)}
                    placeholder="1"
                    className="w-14 px-2 py-1 text-xs border border-border dark:border-white/8 rounded-md bg-surface-2 dark:bg-white/[0.03] focus:outline-none focus:border-primary"
                  />
                </div>
              </div>
            </div>
          ...
          {connections.length === 0 ? (
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                <div className="surface-panel-soft inline-flex items-center justify-center w-9 h-9 rounded-full text-primary shrink-0">
      ...
      <Card>
        ...
        {!!syncModelsError && (
          <p className="text-xs text-red-500 mb-3 break-words">{syncModelsError}</p>
        )}
        {!!modelsTestError && (

...
function PassthroughModelRow({ modelId, fullModel, copied, onCopy, onDeleteAlias, onTest, testStatus, isTesting }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-lg border p-3 transition-colors ${borderColor} hover:bg-white/[0.04]`}>
      <span
      ...
      <div className="flex-1 min-w-0">
        ...
        <code className="rounded bg-white/[0.05] px-1.5 py-0.5 font-mono text-xs text-text-muted">{fullModel}</code>
          <div className="relative group/btn">
            <button
              onClick={() => onCopy(fullModel, `model-${modelId}`)}
          className="rounded p-0.5 text-text-muted transition-colors hover:bg-white/[0.05] hover:text-primary"
            >
      ...
      <div className="flex items-end gap-2">
        <div className="flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40"
          />
        </div>

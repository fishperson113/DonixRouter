...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  return (
    <div className={`group flex flex-col gap-3 rounded-lg p-2 transition-colors hover:bg-white/[0.04] sm:flex-row sm:items-center sm:justify-between ${connection.isActive === false ? "opacity-60" : ""}`}>
      <div className="flex w-full min-w-0 flex-1 items-start gap-3 sm:items-center">
        <div className="flex flex-col">
          <button onClick={onMoveUp} disabled={isFirst} className={`rounded p-0.5 ${isFirst ? "cursor-not-allowed text-text-muted/30" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`}>
            <span className="material-symbols-outlined text-sm">keyboard_arrow_up</span>
          </button>
          <button onClick={onMoveDown} disabled={isLast} className={`rounded p-0.5 ${isLast ? "cursor-not-allowed text-text-muted/30" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {hasAnyProxy && (
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {maskedProxyUrl && <code className="text-[10px] font-mono bg-white/[0.04] px-1 py-0.5 rounded text-text-muted">{maskedProxyUrl}</code>}
          ...
          {(proxyPools || []).length > 0 && (
            <div className="relative" ref={proxyDropdownRef}>
              <button
                ...
                className={`flex flex-col items-center rounded px-2 py-1 transition-colors hover:bg-white/[0.05] ${hasAnyProxy ? "text-primary" : "text-text-muted hover:text-primary"}`}
              ...
              {showProxyDropdown && (
                <div className="surface-panel absolute right-0 top-full z-50 mt-1 min-w-[160px] rounded-lg py-1 shadow-lg">
                  <button onClick={() => handleSelectProxy("__none__")} className={`w-full px-3 py-1.5 text-left text-sm hover:bg-white/[0.05] ${!boundProxyPoolId ? "text-primary font-medium" : "text-text-main"}`}>None</button>
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => handleSelectProxy(pool.id)} className={`w-full px-3 py-1.5 text-left text-sm hover:bg-white/[0.05] ${boundProxyPoolId === pool.id ? "text-primary font-medium" : "text-text-main"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
          )}
          <button onClick={onEdit} className="flex flex-col items-center rounded px-2 py-1 text-text-muted hover:bg-white/[0.05] hover:text-primary">
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input className="w-full rounded-lg border border-white/45 bg-white/74 px-3 py-2 text-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] focus:outline-none focus:border-primary dark:border-white/8 dark:bg-[rgba(255,255,255,0.035)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Production Key" />
        </div>
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-text-muted mb-1 block">API Key</label>
            <input type="password" className="w-full rounded-lg border border-white/45 bg-white/74 px-3 py-2 text-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] focus:outline-none focus:border-primary dark:border-white/8 dark:bg-[rgba(255,255,255,0.035)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]" value={formData.apiKey} onChange={(e) => setFormData({ ...formData, apiKey: e.target.value })} />
          </div>
          <div className="pt-6">
        ...
        <div>
          ...
          <input type="number" className="w-full rounded-lg border border-white/45 bg-white/74 px-3 py-2 text-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] focus:outline-none focus:border-primary dark:border-white/8 dark:bg-[rgba(255,255,255,0.035)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: Number.parseInt(e.target.value) || 1 })} />
        </div>
...
export default function ConnectionsCard({ providerId, isOAuth }) {
  ...
  if (loading) return <Card><div className="h-20 animate-pulse bg-white/[0.05] rounded-lg" /></Card>;
            ...
            {providerStrategy === "round-robin" && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                <input
                  ...
                  className="w-16 rounded-md border border-white/45 bg-white/74 px-2 py-1 text-xs shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] focus:outline-none focus:border-primary dark:border-white/8 dark:bg-[rgba(255,255,255,0.035)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
                />
              </div>
            )}
          </div>

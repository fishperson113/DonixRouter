...
export function ModelRow({ model, fullModel, copied, onCopy, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting }) {
  const borderColor = testStatus === "ok" ? "border-green-500/40" : testStatus === "error" ? "border-red-500/40" : "border-white/8";
  const iconColor = testStatus === "ok" ? "#22c55e" : testStatus === "error" ? "#ef4444" : undefined;

  return (
    <div className={`group rounded-lg border ${borderColor} px-3 py-2 hover:bg-white/[0.04]`}>
      <div className="flex items-center gap-2">
        <span className="material-symbols-outlined text-base" style={iconColor ? { color: iconColor } : undefined}>
          {testStatus === "ok" ? "check_circle" : testStatus === "error" ? "cancel" : "smart_toy"}
        </span>
        <div className="flex flex-col gap-1">
          <code className="text-xs text-text-muted font-mono bg-white/[0.04] px-1.5 py-0.5 rounded">{fullModel}</code>
          {model.name && <span className="text-[9px] text-text-muted/70 italic pl-1">{model.name}</span>}
        </div>
        {onTest && (
          <div className="relative group/btn">
            <button onClick={onTest} disabled={isTesting} className={`rounded p-0.5 text-text-muted transition-opacity hover:bg-white/[0.05] hover:text-primary ${isTesting ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
        ...
        <div className="relative group/btn">
          <button onClick={() => onCopy(fullModel, `model-${model.id}`)} className="rounded p-0.5 text-text-muted hover:bg-white/[0.05] hover:text-primary">
    ...
    <Modal isOpen={isOpen} title="Add Custom Model" onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input
            className="w-full rounded-lg border border-white/45 bg-white/74 px-3 py-2 text-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] focus:outline-none focus:border-primary dark:border-white/8 dark:bg-[rgba(255,255,255,0.035)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
            value={modelId}
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="surface-panel-soft flex items-center gap-1.5 rounded-lg border border-dashed border-white/10 px-3 py-2 text-xs text-text-muted transition-colors hover:border-primary/40 hover:text-primary"
          >

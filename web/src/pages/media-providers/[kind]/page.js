...
function MediaProviderCard({ provider, kind, connections, isCustom, onToggle }) {
  ...
  return (
      <Link href={`/dashboard/media-providers/${kind}/${provider.id}`} className="group">
        <Card
          padding="xs"
          className={`h-full cursor-pointer transition-colors hover:bg-white/[0.04] ${allDisabled ? "opacity-50" : ""}`}
        >
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {allProviders.length === 0 ? (
        <div className="surface-empty rounded-xl py-12 text-center text-sm text-text-muted">

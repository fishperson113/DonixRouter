  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        {providers.length === 0 ? (
          <div className="surface-empty rounded-lg py-6 text-center text-sm text-text-muted">
            No providers yet.
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {providers.map((entry, idx) => {
              ...
              return (
                <div key={`${entry}-${idx}`} className="surface-panel-soft flex items-center gap-3 rounded-lg p-2">
                  ...
                  <div className="flex items-center gap-0.5">
                    <button onClick={() => handleMove(idx, -1)} disabled={idx === 0} className={`rounded p-1 transition-colors ${idx === 0 ? "text-text-muted/20" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`} title="Move up">
                      <span className="material-symbols-outlined text-[16px]">arrow_upward</span>
                    </button>
                    <button onClick={() => handleMove(idx, 1)} disabled={idx === providers.length - 1} className={`rounded p-1 transition-colors ${idx === providers.length - 1 ? "text-text-muted/20" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`} title="Move down">
      ...
      {combo.kind && examplePath && (
        <Card>
          ...
          <pre className="surface-code overflow-x-auto whitespace-pre-wrap break-all rounded-lg p-3 text-xs font-mono">
            {curlExample}
          </pre>
          {testError && (
            ...
            <div className="mt-3 flex flex-col gap-3">
              ...
              {testResult.imageUrl && (
                <div>
                  ...
                  <img src={testResult.imageUrl} alt="Generated" className="max-w-full rounded-lg border border-white/8 bg-white/[0.02]" />
                </div>
              )}
              {testResult.audioUrl && (
                <div>
              ...
              {testResult.json && (
                <pre className="surface-code max-h-[300px] overflow-auto whitespace-pre-wrap break-all rounded-lg p-3 text-xs font-mono">
                  {testResult.json}
                </pre>
              )}
            </div>
      ...
      <Card>
        ...
        {logs.length === 0 ? (
          ...
          <pre className="surface-code max-h-[400px] overflow-auto whitespace-pre-wrap rounded-lg p-3 text-[11px] font-mono">
            {logs.join("\n")}
          </pre>
        )}
      </Card>

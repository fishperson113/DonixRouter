...
function ProviderCard({ provider, kind, connections }) {
  ...
  return (
    <Link href={`/dashboard/media-providers/${kind}/${provider.id}`} className="group">
      <Card padding="xs" className={`h-full cursor-pointer transition-colors hover:bg-white/[0.04] ${allDisabled ? "opacity-50" : ""}`}>
    ...
    <div className="flex flex-col gap-2">
      {combos.map((combo) => (
        <Link key={combo.id} href={`/dashboard/media-providers/combo/${combo.id}`}>
          <Card padding="xs" className="cursor-pointer transition-colors hover:bg-white/[0.04]">
            <div className="flex min-w-0 items-center gap-3">
...
function Section({ title, icon, kind, providers, connections, combos, onCreateCombo }) {
  return (
    <div>
      ...
      {providers.length === 0 ? (
        <div className="surface-empty rounded-xl border border-dashed border-white/10 py-8 text-center text-sm text-text-muted">
          No providers.
        </div>
      ) : (

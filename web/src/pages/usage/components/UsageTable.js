...
export default function UsageTable({
  ...
  return (
    <Card className="overflow-hidden">
      <div className="border-b border-white/8 bg-white/[0.02] p-4">
        <h3 className="font-semibold">{title}</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="surface-table w-full text-left text-sm">
          <thead>
            <tr>
              {columns.map((col) => (
                <th
                  key={col.field}
                  className={`cursor-pointer px-6 py-3 hover:bg-white/[0.03] ${col.align === "right" ? "text-right" : ""}`}
              ...
              {valueColumns.map((col) => (
                <th
                  key={col.field}
                  className="cursor-pointer px-6 py-3 text-right hover:bg-white/[0.03]"
                  onClick={() => onToggleSort(tableType, col.field)}
                >
                  {col.label}{" "}
                  <SortIcon field={col.field} currentSort={sortBy} currentOrder={sortOrder} />
          ...
          <tbody>
            {groupedData.map((group) => (
              <Fragment key={group.groupKey}>
                {/* Group summary row */}
                <tr
                  className="group-summary cursor-pointer transition-colors hover:bg-white/[0.03]"
                ...
                {expanded.has(group.groupKey) && group.items.map((item) => (
                  <tr
                    key={`detail-${item.key}`}
                    className="group-detail transition-colors hover:bg-white/[0.02]"
                  >

...
export default function QuotaTable({ quotas = [], compact = false }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="surface-table w-full table-fixed text-left">
        <tbody>
              ...
              >
                ...
                <td className={`${cellPad} w-[30%]`}>
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className={`${nameText} truncate font-medium text-text-main`}>
                      {quota.name}
                    </span>
                  </div>
                </td>
                ...
                <td className={`${cellPad} w-[45%]`}>
                  <div className={compact ? "space-y-1" : "space-y-1.5"}>
                    ...
                    <div className={`${compact ? "h-1" : "h-1.5"} overflow-hidden rounded-full border ${colors.bgLight} ${
                      remaining === 0 ? 'border-white/10' : 'border-transparent'
                    }`}>
                      <div
                ...
                <td className={`${cellPad} w-[25%]`}>
                  {countdown !== "-" || resetDisplay ? (
                    compact ? (
                      <div
                        className={`${resetPrimary} truncate font-medium text-text-main`}
                        title={resetDisplay || ""}
                      >
                      ...
                      <div className="space-y-0.5">
                        {countdown !== "-" && (
                          <div className={`${resetPrimary} font-medium text-text-main`}>
                            in {countdown}
                          </div>
                        )}
                        {resetDisplay && (

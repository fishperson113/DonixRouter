...
const hasDonixRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "DonixRouter");
};

const getDonixRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "DonixRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getDonixRouterEntry(config);
...
// POST - Apply DonixRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing DonixRouter entry or append
    const idx = config.findIndex((e) => e.name === "DonixRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove DonixRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "DonixRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "DonixRouter removed from Copilot config",
    });
  } catch (error) {

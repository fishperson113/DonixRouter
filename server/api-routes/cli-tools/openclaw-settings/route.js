...
// Check if settings has DonixRouter config
const hasDonixRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["donixrouter"];
};

// Read per-agent models.json and return current model id (without "donixrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["donixrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasDonixRouter: hasDonixRouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["donixrouter"] = {
    baseUrl,
...
// POST - Update DonixRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `donixrouter/${model}`;

    // Remove all old donixrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("donixrouter/"))
    ...
    // Add fresh donixrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`donixrouter/${m}`] = {};
    });

    // Remove old donixrouter model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (agent.model?.startsWith("donixrouter/")) {
    ...
    // Update models.providers.donixrouter with all models
    settings.models.providers["donixrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `donixrouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove DonixRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove DonixRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["donixrouter"];
    ...
    // Remove donixrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("donixrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses donixrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("donixrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "DonixRouter settings removed successfully",
    });
  } catch (error) {

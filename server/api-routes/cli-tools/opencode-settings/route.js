...
const hasDonixRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["donixrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["donixrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasDonixRouter: hasDonixRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("donixrouter/") ? config.model.replace(/^donixrouter\//, "") : null,
...
// POST - Apply DonixRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_donixrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing donixrouter provider entry and its models
    const existingProvider = config.provider["donixrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["donixrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `donixrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `donixrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("donixrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove DonixRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["donixrouter"]?.models) {
      delete config.provider["donixrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["donixrouter"].models).length === 0) {
        delete config.provider["donixrouter"];
        if (config.model?.startsWith("donixrouter/")) delete config.model;
      } else if (config.model === `donixrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["donixrouter"].models);
        config.model = `donixrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire donixrouter provider
      if (config.provider) delete config.provider["donixrouter"];
      if (config.model?.startsWith("donixrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("donixrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "DonixRouter settings removed from OpenCode",
    });
  } catch (error) {

...
import { normalizeDeepSeekWebCookieInput } from "#shared/utils/deepseekWebAuth.js";
...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "DonixRouter", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function testOAuthConnection(connection, effectiveProxy = null) {
  if (connection.provider === "deepseek-web") {
    return testDeepSeekWebConnection(connection, effectiveProxy);
  }
...
async function testDeepSeekWebConnection(connection, effectiveProxy = null) {
  const executor = getExecutor("deepseek-web");
  const cookieHeader = normalizeDeepSeekWebCookieInput(
    connection.providerSpecificData?.webCookie ||
    connection.providerSpecificData?.cookie ||
    (connection.authType === "cookie" ? connection.apiKey : ""),
  );

  const current = await executor.fetchCurrentUser({
  ...
  const refreshed = await executor.refreshCredentials({
    ...
    providerSpecificData: {
      ...
      ...(cookieHeader ? { webCookie: cookieHeader, cookie: cookieHeader } : {}),
    },
  }, null, effectiveProxy);
  ...
  if (!refreshed) {
    return {
      valid: false,
      error: cookieHeader
        ? "DeepSeek cookie is invalid or expired"
        : "DeepSeek token is invalid or expired",
      refreshed: false,
      newTokens: null,
    };
  }

  const retry = await executor.fetchCurrentUser({
    accessToken: refreshed.accessToken || connection.accessToken || "",
    cookieHeader: normalizeDeepSeekWebCookieInput(
      refreshed.providerSpecificData?.webCookie ||
      refreshed.providerSpecificData?.cookie ||
      cookieHeader,
    ),
  }, null, effectiveProxy);
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "kiro-web": {
        const cookie = normalizeKiroWebCookieInput(connection.apiKey);
        const res = await fetchWithConnectionProxy("https://app.kiro.dev/", {
          method: "GET",
          headers: {
        ...
        const valid = res.ok && isKiroWebSessionValid(analysis);
        return { valid, error: valid ? null : getKiroWebSessionError(analysis) };
      }
      case "perplexity-web": {

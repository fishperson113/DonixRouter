...
import { normalizeDeepSeekWebCookieInput, normalizeDeepSeekWebTokenInput } from "#shared/utils/deepseekWebAuth.js";
import { analyzeKiroWebSessionHtml, getKiroWebSessionError, isKiroWebSessionValid, normalizeKiroWebCookieInput } from "#shared/utils/kiroWebAuth.js";
  ...
  try {
    ...
    try {
      ...
      switch (provider) {
        ...
        case "kiro-web": {
          const cookie = normalizeKiroWebCookieInput(apiKey);
          const res = await fetch("https://app.kiro.dev/", {
            method: "GET",
            headers: {
          ...
          const analysis = analyzeKiroWebSessionHtml(html);
          isValid = res.ok && isKiroWebSessionValid(analysis);
          if (!isValid) {
            error = getKiroWebSessionError(analysis);
          }
          break;
        }
        ...
        case "deepseek-web": {
          ...
          const rawCookie = normalizeDeepSeekWebCookieInput(apiKey);
          const token = normalizeDeepSeekWebTokenInput(apiKey);
          const auth =
            rawCookie && rawCookie.includes("=")
              ? await executor.loginWithCookie(rawCookie, null, null, AbortSignal.timeout(8000))
              : token
                ? await executor.loginWithToken(token, null, null, AbortSignal.timeout(8000))
                : await executor.fetchCurrentUser({ accessToken: apiKey }, null, null, AbortSignal.timeout(8000));

          isValid = !!auth?.ok || !!auth?.cookieHeader || !!auth?.profile?.accountId;
          if (!isValid) {
            error = rawCookie && rawCookie.includes("=")
              ? (auth?.data?.code === 40002 || /missing token/i.test(auth?.data?.msg || "")
                  ? "DeepSeek cookie is missing the auth token. aws-waf-token or ds_session_id alone is not enough; paste the full logged-in cookie or use Web Login with userToken."
                  : "Invalid DeepSeek cookie — paste the full cookie string from chat.deepseek.com")
              : "Invalid DeepSeek token — re-login or paste a fresh cookie";
          }
          break;
        }

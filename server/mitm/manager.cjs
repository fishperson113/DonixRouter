...
const { addDNSEntry, removeDNSEntry, removeAllDNSEntries, removeAllDNSEntriesSync, checkAllDNSStatus, TOOL_HOSTS, isSudoAvailable, isSudoPasswordRequired } = require("./dns/dnsConfig.cjs");
const { isAdmin } = require("./winElevated.cjs");

const IS_WIN = process.platform === "win32";
const IS_MAC = process.platform === "darwin";
const { generateCert } = require("./cert/generate.cjs");
const { installCert, uninstallCert } = require("./cert/install.cjs");
const { isCertExpired } = require("./cert/rootCA.cjs");
const { DATA_DIR, MITM_DIR } = require("./paths.cjs");
const { log, err } = require("./logger.cjs");
...
async function scheduleMitmRestart(apiKey) {
  ...
  // If server is actually still running, no restart needed
  if (serverProcess && !serverProcess.killed) {
    log("Server still running — skipping restart");
    return;
  }
  ...
  try {
    ...
    mitmIsRestarting = false;
    // If server is already running, no point retrying
    if (e.message && e.message.includes("already running")) {
      log("Server is already running — stopping restart loop");
      mitmRestartCount = 0;
      return;
    }
    // Schedule next retry
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (serverProcess) {
    ...
    serverProcess.on("exit", (code) => {
      ...
      // But first check if another instance is already running
      if (code !== 0 && !mitmIsRestarting) {
        setTimeout(async () => {
          try {
            const health = await pollMitmHealth(2000, MITM_PORT);
            if (health && health.ok) {
              log("Server health check passed, another instance is running. No restart needed.");
              return;
            }
            scheduleMitmRestart(apiKey);
          } catch (e) {
...
async function stopServer(sudoPassword) {
  ...
  if (IS_WIN) {
    ...
    try {
      const { isAdmin, runElevatedPowerShell, quotePs } = require("./winElevated.cjs");
      if (isAdmin()) {

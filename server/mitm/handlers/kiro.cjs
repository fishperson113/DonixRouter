      ...
      for (const line of lines) {
        ...
        if (delta.tool_calls) {
          ...
          for (const tc of delta.tool_calls) {
            ...
            // Accumulate arguments as string — OpenAI SSE always sends strings
            const argType = typeof tc.function?.arguments;
            if (tc.function?.arguments != null) {
              // OpenAI SSE sends arguments as string chunks, just concatenate
              acc.args += String(tc.function.arguments);
            }

  ...
  try {
    const response = await fetch(`${ROUTER_BASE}${path}`, {
      ...
      headers: {
        ...
        // Mark as internal request to prevent MITM loop
    ...
    if (!response.ok) {
      err(`[fetchRouter] ${path} returned ${response.status}: ${response.statusText}`);
    }

    // Forward response as-is (status + body). pipeSSE will propagate status.
    return response;
  } catch (error) {
    err(`[fetchRouter] Failed to connect to router at ${ROUTER_BASE}${path}: ${error.message}`);
    throw new Error(`Router connection failed: ${error.message}. Is the router running on ${ROUTER_BASE}?`);
  }
}

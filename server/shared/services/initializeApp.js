...
} from "#lib/tunnel/tunnelManager.js";
import { killCloudflared, isCloudflaredRunning, ensureCloudflared } from "#lib/tunnel/cloudflared.js";
import { isTailscaleRunning } from "#lib/tunnel/tailscale.js";
import { loadState } from "#lib/tunnel/state.js";
import { checkInternet, probeUrlAlive } from "#lib/tunnel/networkProbe.js";
import {
  RESTART_COOLDOWN_MS, NETWORK_SETTLE_MS,
  WATCHDOG_INTERVAL_MS, NETWORK_CHECK_INTERVAL_MS,
} from "#lib/tunnel/tunnelConfig.js";
import { getMitmStatus, startMitm, loadEncryptedPassword, initDbHooks, restoreToolDNS, removeAllDNSEntriesSync } from "#mitm/manager-esm.js";
...
async function autoStartMitm() {
  ...
  try {
    ...
    await startMitm(activeKey?.key || "sk_donixrouter", password);
...
async function safeRestartTunnel(reason) {
  ...
  if (isCloudflaredRunning()) {
    ...
    const publicUrl = state?.shortId ? `https://r${state.shortId}.donixrouter.com` : null;

...
export default function NineRemotePromoModal({ isOpen, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="surface-panel relative flex w-full max-w-sm flex-col overflow-hidden rounded-[14px] border border-white/10 shadow-[var(--shadow-elev)] fade-in">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/8 px-5 py-3">
          ...
          <button
            onClick={onClose}
            className="rounded-[10px] p-1.5 text-text-muted transition-colors hover:bg-white/[0.06] hover:text-text-main"
          >
        ...
        <div className="px-7 py-7 pb-9 flex flex-col gap-6">
          ...
          <div className="flex gap-2 w-full">
            {FEATURES.map(({ icon, label, desc }) => (
              <div key={label} className="surface-panel-soft flex flex-1 flex-col items-center gap-1.5 rounded-[10px] border border-white/10 px-1 py-4">

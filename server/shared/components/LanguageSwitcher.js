...
import { LOCALES, LOCALE_COOKIE, normalizeLocale } from "#i18n/config.js";
import { reloadTranslations } from "#i18n/runtime.js";
    ...
    <div className={className}>
      ...
      {!hideTrigger && (
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 rounded-lg px-3 py-2 text-text-muted transition-colors hover:bg-white/[0.05] hover:text-text-main"
          title="Language"
          data-i18n-skip="true"
        >
      ...
      {isOpen && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" data-i18n-skip="true">
          ...
          <div
            ref={modalRef}
            className="surface-panel relative flex max-h-[80vh] w-full max-w-2xl flex-col rounded-xl border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200"
          >
            {/* Modal header */}

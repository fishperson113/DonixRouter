...
import { Modal, Button, Input, OAuthModal } from "#shared/components.js";
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="surface-panel-soft flex flex-col items-center gap-2 rounded-lg border border-white/10 p-4 text-left transition-colors hover:border-primary/40 hover:bg-primary/10"
              >
        ...
        {mode === "oauth" && (
          <>
            <p className="text-xs text-text-muted">
              ...
              with redirect URI{" "}
              <code className="rounded bg-white/[0.05] px-1 text-xs">{getRedirectUri()}</code>
            </p>
        ...
        {mode === "pat" && (
          <>
            <p className="text-xs text-text-muted">
              ...
              with scopes: <code className="rounded bg-white/[0.05] px-1 text-xs">api</code>,{" "}
              <code className="rounded bg-white/[0.05] px-1 text-xs">read_user</code>, and{" "}
              <code className="rounded bg-white/[0.05] px-1 text-xs">ai_features</code>.
            </p>
            <Input label="GitLab Base URL" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder={GITLAB_COM} />

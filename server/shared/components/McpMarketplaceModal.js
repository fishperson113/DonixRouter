      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-white/10 bg-white/[0.04] px-2 py-1.5 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40"
          />
          <select
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            className="rounded-lg border border-white/10 bg-white/[0.04] px-2 py-1.5 text-xs text-text-main shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40"
          >
        ...
        {!loading && (
          <div className="flex flex-col gap-1 max-h-[60vh] overflow-y-auto">
            ...
            {filtered.map((s) => {
              ...
              return (
                <div key={s.url} className="rounded-lg border border-transparent transition-colors hover:border-white/10">
                  <div className="flex items-start gap-2 rounded-lg px-2 py-2 transition-colors hover:bg-white/[0.05]">
                    {s.iconUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={s.iconUrl} alt="" className="size-7 rounded shrink-0 object-contain" onError={(e) => { e.target.style.display = "none"; }} />
                    ) : (
                      <div className="surface-panel-soft size-7 shrink-0 rounded" />
                    )}
                    ...
                    <button
                      ...
                      className={`shrink-0 px-2 py-1 rounded text-[10px] font-medium transition-colors ${
                        added
                          ...
                          : expanded
                          ? "border border-white/10 bg-white/[0.04] text-text-muted hover:bg-white/[0.06]"
                  ...
                  {expanded && (
                    <div className="surface-panel-soft flex flex-col gap-2 border-t border-white/8 px-3 py-2">
                      {isLoadingTools && (
                      ...
                      {!isLoadingTools && toolKeys.length > 0 && (
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto">
                            {toolKeys.map((t) => (
                              <label key={t} className="flex cursor-pointer items-center gap-1.5 rounded px-1 text-[10px] transition-colors hover:bg-white/[0.05]">
                                <input
                                  type="checkbox"
                                  checked={!!sel[t]}

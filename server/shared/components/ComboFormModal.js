  ...
  return (
    <div className="group flex min-w-0 items-center gap-1.5 rounded-md bg-black/[0.02] px-2 py-1 transition-colors hover:bg-black/[0.04] dark:bg-white/[0.02] dark:hover:bg-white/[0.04]">
      ...
      {editing ? (
        ...
        <div className="min-w-0 flex-1 cursor-text truncate rounded px-1.5 py-0.5 font-mono text-xs text-text-main transition-colors hover:bg-white/[0.05]"
          onClick={() => setEditing(true)} title="Click to edit">{model}</div>
      )}
      <div className="flex shrink-0 items-center gap-0.5">
        <button onClick={onMoveUp} disabled={isFirst}
          className={`rounded p-0.5 ${isFirst ? "cursor-not-allowed text-text-muted/20" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`} title="Move up">
          <span className="material-symbols-outlined text-[12px]">arrow_upward</span>
        </button>
        <button onClick={onMoveDown} disabled={isLast}
          className={`rounded p-0.5 ${isLast ? "cursor-not-allowed text-text-muted/20" : "text-text-muted hover:bg-white/[0.05] hover:text-primary"}`} title="Move down">
        ...
        <div className="flex flex-col gap-3">
          <div>
            {forcePrefix ? (
              ...
              <span className="inline-flex items-center rounded-l border border-r-0 border-white/10 bg-white/[0.04] px-2 font-mono text-sm text-text-muted">{forcePrefix}</span>
                  <input value={name} onChange={handleNameChange} placeholder="my-combo"
              className="min-w-0 flex-1 rounded-r border border-white/10 bg-white/[0.04] px-2 py-1.5 font-mono text-sm text-text-main outline-none shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] focus:border-primary/40" />
                </div>
          ...
          <div className="surface-empty rounded-lg py-4 text-center">
            ...
            className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-white/10 py-2 text-xs font-medium text-primary transition-colors hover:border-primary/50 hover:bg-primary/10 hover:text-primary">

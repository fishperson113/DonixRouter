...
export const AI_MODELS = Object.entries(MODELS).flatMap(([alias, models]) =>
  models.map(m => {
    const entry = { provider: alias, model: m.id, name: m.name };
    if (m.type) entry.type = m.type;
    if (m.kinds) entry.kinds = m.kinds;
    if (m.params) entry.params = m.params;
    if (m.capabilities) entry.capabilities = m.capabilities;
    if (m.strip) entry.strip = m.strip;
    if (m.upstreamModelId) entry.upstreamModelId = m.upstreamModelId;
    if (m.targetFormat) entry.targetFormat = m.targetFormat;
    if (m.quotaFamily) entry.quotaFamily = m.quotaFamily;
    return entry;
  })
);

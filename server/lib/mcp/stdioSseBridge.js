...
import { LOCAL_STDIO_PLUGINS } from "#shared/constants/coworkPlugins.js";
import { DATA_DIR } from "#lib/dataDir.js";

const CUSTOM_FILE = path.join(DATA_DIR, "mcp", "customPlugins.json");

const G_KEY = "__donixrouterMcpBridges";
...
function smartFilterText(text) {
  ...
  if (out.length > MAX_TEXT_CHARS) {
    ...
    out = `${head}\n\n... [truncated ${text.length - head.length} chars by donixrouter bridge. Page is large; ask user to scroll/navigate to a specific section, or click an element with the refs shown above]`;
  }
  return out;
}
...
function collapseRepeated(text) {
  ...
  while (i < lines.length) {
    ...
    if (groupLen >= COLLAPSE_THRESHOLD) {
      ...
      out.push(`${indent}... [${groupLen - COLLAPSE_KEEP_HEAD - COLLAPSE_KEEP_TAIL} similar "${role}" items omitted by donixrouter bridge]`);
...
const getCustomStore = () => {
  if (!globalThis.__donixrouterCustomPlugins) globalThis.__donixrouterCustomPlugins = new Map();
  return globalThis.__donixrouterCustomPlugins;
};

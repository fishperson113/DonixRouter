...
/** Connection IDs with at least one in-flight API request (for quota widget highlight). */
export function getActiveConnectionIds() {
  const ids = [];
  for (const [connectionId, models] of Object.entries(pendingRequests.byAccount)) {
    for (const count of Object.values(models)) {
      if (count > 0) {
...
export async function getUsageStats(period = "all") {
  ...
  const recentRows = db.all(
    `SELECT timestamp, provider, model, tokens, status, cost FROM usageHistory ORDER BY id DESC LIMIT 100`
  );
  const seen = new Set();
...
function resolveChartOptions(options) {
  if (typeof options === "string") {
    return {
      range: options,
      granularity: options === "24h" ? "hourly" : "daily",
    };
  }

  const range = options?.range || options?.period || "24h";
  let granularity = options?.granularity || (range === "24h" ? "hourly" : "daily");

  if (!CHART_GRANULARITY_MS[granularity]) {
    granularity = range === "24h" ? "hourly" : "daily";
  }

  if (!Object.prototype.hasOwnProperty.call(CHART_RANGE_MS, range)) {
    return { range: "24h", granularity: "hourly" };
  }

  if (range === "all" && granularity === "five_min") {
    granularity = "daily";
  }
...
function alignBucketStart(timestamp, bucketMs) {
...
export async function getChartData(options = "7d") {
  const db = await getAdapter();
  const { range, granularity } = resolveChartOptions(options);
  const bucketMs = CHART_GRANULARITY_MS[granularity];
  const now = Date.now();

  let startTime = CHART_RANGE_MS[range] == null ? null : now - CHART_RANGE_MS[range];
  if (startTime == null) {
    const oldest = db.get(`SELECT timestamp FROM usageHistory ORDER BY timestamp ASC LIMIT 1`);
    startTime = oldest?.timestamp ? new Date(oldest.timestamp).getTime() : now - CHART_RANGE_MS["24h"];
  }

  const alignedStart = alignBucketStart(startTime, bucketMs);
  const alignedEnd = alignBucketStart(now, bucketMs);
  const buckets = [];
  ...
  for (let ts = alignedStart; ts <= alignedEnd; ts += bucketMs) {
    const bucket = {
      ...
      label: formatChartLabel(ts, granularity),
      input_tokens: 0,
      output_tokens: 0,
      cached_tokens: 0,
      request_count: 0,
  ...
  const rows = db.all(
    `SELECT timestamp, promptTokens, completionTokens, tokens FROM usageHistory WHERE timestamp >= ? ORDER BY timestamp ASC`,
    [new Date(alignedStart).toISOString()]
  );

  for (const row of rows) {
    const rowTs = new Date(row.timestamp).getTime();
    if (!Number.isFinite(rowTs) || rowTs < alignedStart || rowTs > now) continue;

    const bucketTs = alignBucketStart(rowTs, bucketMs);
    const bucket = bucketMap.get(bucketTs);
    if (!bucket) continue;

    const tokenPayload = parseJson(row.tokens, {}) || {};
    const inputTokens = row.promptTokens || tokenPayload.prompt_tokens || tokenPayload.input_tokens || 0;
    const outputTokens = row.completionTokens || tokenPayload.completion_tokens || tokenPayload.output_tokens || 0;
    const cachedTokens = tokenPayload.cached_tokens || tokenPayload.cache_read_input_tokens || 0;

    bucket.input_tokens += inputTokens;
    bucket.output_tokens += outputTokens;
    bucket.cached_tokens += cachedTokens;
...
async function getChartDataLegacy(period = "7d") {
  const db = await getAdapter();
  const now = Date.now();

  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "donixrouter")
        : path.join(os.homedir(), ".donixrouter"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all donixrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from donixrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("donixrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("donixrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "donixrouter");
  }
  return path.join(os.homedir(), ".donixrouter");
}
...
// Resolve npx/donixrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx donixrouter` — works regardless of global bin path changes after npm i -g

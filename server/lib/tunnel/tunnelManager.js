...
const WORKER_URL = process.env.TUNNEL_WORKER_URL || "https://donixrouter.com";
const MACHINE_ID_SALT = "donixrouter-tunnel-salt";
...
function getMachineId() {
  try {
    const { machineIdSync } = _require("node-machine-id");
  ...
  try {
    if (isCloudflaredRunning()) {
      ...
      if (existing?.tunnelUrl && await probeUrlAlive(existing.tunnelUrl)) {
        const publicUrl = `https://r${existing.shortId}.donixrouter.com`;
    ...
    const publicUrl = `https://r${shortId}.donixrouter.com`;
    ...
    // Then verify public URL (DNS propagated through donixrouter.com worker)
...
export async function getTunnelStatus() {
  ...
  const publicUrl = shortId ? `https://r${shortId}.donixrouter.com` : "";

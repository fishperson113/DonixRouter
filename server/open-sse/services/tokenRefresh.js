...
function getRefreshCacheKey(provider, refreshToken, credentials = null) {
  if (refreshToken) return `${provider}:${refreshToken}`;
  if (provider === "kiro") {
    return `${provider}:${getKiroWebRefreshCacheToken(credentials)}`;
  }
  return `${provider}:`;
}
...
function getCookieValue(cookieHeader, name) {
  ...
  const pair = parseCookiePairs(cookieHeader).find((item) => item.name.toLowerCase() === lowerName);
  return pair?.value || "";
}

function normalizeKiroIdp(providerSpecificData = {}) {
  const data = providerSpecificData || {};
  const raw = data.idp || data.provider || data.authMethod || "Google";
  return String(raw).toLowerCase().includes("github") ? "Github" : "Google";
}

function resolveKiroWebCookieHeader(refreshToken, providerSpecificData = {}, accessTokenCookie = "") {
  const data = providerSpecificData || {};
  const raw = data.cookieHeader
    || data.webCookie
    || data.fullCookie
    || data.cookies
    || "";
  ...
  const idp = normalizeKiroIdp(data);
  const parts = [];
  if (accessTokenCookie) parts.push(`AccessToken=${accessTokenCookie}`);
  if (refreshToken) parts.push(`RefreshToken=${refreshToken}`);
  parts.push(`Idp=${idp}`);
  if (data.userId || data.kiroUserId) {
    parts.push(`UserId=${data.userId || data.kiroUserId}`);
  }
  if (data.visitorId || data.kiroVisitorId) {
    parts.push(`kiro-visitor-id=${data.visitorId || data.kiroVisitorId}`);
  }
  return parts.join("; ");
}

function resolveKiroWebRefreshContext(refreshToken, providerSpecificData = {}) {
  const data = providerSpecificData || {};
  const profileArn = data.profileArn;
  if (!profileArn) return null;

  const explicitCookieHeader = data.cookieHeader
    || data.webCookie
    || data.fullCookie
    || data.cookies
    || "";
  const accessTokenCookie = data.webAccessToken
    || data.portalAccessToken
    || data.cookie
    || getCookieValue(explicitCookieHeader, "AccessToken");
  const refreshTokenCookie = getCookieValue(explicitCookieHeader, "RefreshToken") || refreshToken || "";

  if (!accessTokenCookie) return null;

  const cookieHeader = resolveKiroWebCookieHeader(refreshTokenCookie, data, accessTokenCookie);
  const idp = normalizeKiroIdp(data);
  const userId = data.userId
    || data.kiroUserId
    || getCookieValue(cookieHeader, "UserId")
    || "";
  const visitorId = data.visitorId
    || data.kiroVisitorId
    || getCookieValue(cookieHeader, "kiro-visitor-id")
    || "";
...
function canRefreshKiroWithWebCookie(credentials = {}) {
  return !!resolveKiroWebRefreshContext(credentials.refreshToken, credentials.providerSpecificData);
}

function getKiroWebRefreshCacheToken(credentials = {}) {
  const context = resolveKiroWebRefreshContext(credentials.refreshToken, credentials.providerSpecificData);
  return context?.refreshTokenCookie || context?.accessTokenCookie || context?.cookieHeader || "";
}
...
export async function refreshKiroToken(refreshToken, providerSpecificData, log, proxyOptions = null) {
  const webContext = resolveKiroWebRefreshContext(refreshToken, providerSpecificData);
  if (webContext) {
    try {
      const service = new KiroService();
      const fetchFn = (url, init) => proxyAwareFetch(url, init, proxyOptions);
      let csrf = webContext.csrfToken;

      if (!csrf) {
        const csrfResult = await service.fetchCsrf(webContext.accessTokenCookie, webContext.idp, {
          fetchFn,
          cookieHeader: webContext.cookieHeader,
        });
        csrf = csrfResult.csrf;
      }

      const tokens = await service.getTokenCbor(
        webContext.accessTokenCookie,
        webContext.idp,
        csrf,
        { profileArn: webContext.profileArn },
        {
          fetchFn,
          cookieHeader: webContext.cookieHeader,
          userId: webContext.userId,
          visitorId: webContext.visitorId,
        }
      );

      log?.info?.("TOKEN_REFRESH", "Successfully refreshed Kiro web token", {
      ...
      return {
        ...
        refreshToken: tokens.refreshToken || webContext.refreshTokenCookie || refreshToken,
        expiresIn: tokens.expiresIn,
        providerSpecificData: {
          ...(providerSpecificData || {}),
          profileArn: tokens.profileArn || webContext.profileArn,
          csrfToken: tokens.csrfToken || csrf,
          cookieHeader: webContext.cookieHeader,
          ...(webContext.userId ? { userId: webContext.userId } : {}),
          ...(webContext.visitorId ? { visitorId: webContext.visitorId } : {}),
        },
      };
    } catch (error) {
      log?.error?.("TOKEN_REFRESH", "Failed to refresh Kiro web token", {
        error: error.message,
      });
  ...
  if (!refreshToken) {
    log?.warn?.("TOKEN_REFRESH", "No Kiro refresh token or web cookie available");
    return null;
  }
  ...
  return {
    ...
    refreshToken: tokens.refreshToken || refreshToken,
    expiresIn: tokens.expiresIn,
    providerSpecificData: tokens.profileArn
      ? { ...(providerSpecificData || {}), profileArn: tokens.profileArn }
      : undefined,
  };
}
...
export async function getAccessToken(provider, credentials, log) {
  ...
  const hasKiroWebCookie = provider === "kiro" && canRefreshKiroWithWebCookie(credentials);

  if (!credentials || (!hasRefreshToken && !hasKiroWebCookie)) {
    log?.warn?.("TOKEN_REFRESH", `No valid refresh token available for provider: ${provider}`);
    return null;
  }

  const cacheKey = getRefreshCacheKey(provider, credentials.refreshToken, credentials);
...
export async function refreshTokenByProvider(provider, credentials, log) {
  if (!credentials.refreshToken && !(provider === "kiro" && canRefreshKiroWithWebCookie(credentials))) return null;

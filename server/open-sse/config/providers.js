...
export const PROVIDERS = {
  ...
  codex: {
    ...
    // Upstream "servers overloaded" (503/529) is transient — retry before rotating accounts
    retry: { 500: { attempts: 2, delayMs: 1500 }, 503: { attempts: 3, delayMs: 2000 }, 529: { attempts: 3, delayMs: 2000 } },
  },
  "deepseek-web": {
  ...
  kiro: {
    ...
    format: "kiro",
    // Kiro upstream occasionally returns 500 "Encountered an unexpected error... please try again"
    // and the message itself asks the client to retry. Retry on the same account a few times
    // before letting account-fallback rotate to the next connection.
    retry: { 429: 2, 500: { attempts: 2, delayMs: 1500 } },
    headers: {
      "Content-Type": "application/json",

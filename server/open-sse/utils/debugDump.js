 ...
 *
 * JavaScript port of codex-proxy-dev/src/utils/debug-dump.ts
 */
...
export function debugDumpEnabled() {
  return ENABLED;
}

export function debugDumpPath() {
  return DUMP_PATH;
}

export function debugDump(kind, payload) {
  if (!DUMP_PATH) return;
  if (!announced) {
    announced = true;

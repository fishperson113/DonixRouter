...
export function handleStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, userAgent, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, streamController, onStreamComplete }) {
  // Call onRequestSuccess early for success signal; affinity recording happens in onStreamComplete
  if (onRequestSuccess) onRequestSuccess();

  // Wrap onStreamComplete to also record session affinity metadata
  const wrappedOnStreamComplete = onStreamComplete ? (contentObj, usage, ttftAt, meta) => {
    onStreamComplete(contentObj, usage, ttftAt, meta);
    // Trigger affinity recording with responseId if available
    if (meta?.responseId && onRequestSuccess) {
      onRequestSuccess({ responseId: meta.responseId, inputTokens: usage?.prompt_tokens });
    }
  } : null;

  const transformStream = buildTransformStream({ provider, sourceFormat, targetFormat, userAgent, reqLogger, toolNameMap, model, connectionId, body, onStreamComplete: wrappedOnStreamComplete, apiKey });

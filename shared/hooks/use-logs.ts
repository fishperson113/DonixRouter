...
export function useLogs(refreshIntervalMs = 1500) {
  ...
  useEffect(() => {
    ...
    // SSE replaces the bulk of the polling. Records flushes and state
    // changes trigger an immediate refetch with the current filter/page
    // context. A light fallback interval is kept so the table self-heals
    // even when SSE blackholes (proxy, dev-tools network panel stall).
    ...
    try {
      ...
      es.onmessage = (ev) => {
        try {
          const msg = JSON.parse(ev.data) as { type: string; state?: LogState };
          if (msg.type === "records") {
            if (!document.hidden && page === 0) load(page);
    ...
    } catch { /* environment without EventSource — fallback only */ }

    timerRef.current = setInterval(tick, refreshIntervalMs * 10);
    const onVisibility = () => {
